...
class SteelHistoricalReplayAdapter:
    """
    Adapts point-in-time cached price store statistics into the format
    expected by SteelInefficiencyDetector.
    """
    def __init__(self, config: CommodityReplayConfig, detector: Optional[SteelInefficiencyDetector] = None):
        if not isinstance(config, CommodityReplayConfig):
            raise TypeError("config must be a CommodityReplayConfig instance")
        self.config = config
        self.detector = detector if detector is not None else SteelInefficiencyDetector(
            gap_threshold=config.convergence_gap_threshold,
