...
class GoldEpisodeDatasetWriter:
    """
    Thread-safe, append-only JSONL dataset writer for closed Gold inefficiency episodes.
    """
    def __init__(self, dataset_path: str = "storage/gold_inefficiency_episodes.jsonl"):
        ...
        if self.dataset_path.exists():
            with open(self.dataset_path, "r", encoding="utf-8") as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    if episode.get("episode_type") != "gold_inefficiency_episode":
                        raise ValueError(f"Record on line {line_num} episode_type must be 'gold_inefficiency_episode'")
    ...
    def write_episode(self, episode: GoldInefficiencyEpisode) -> Dict[str, Any]:
        if not isinstance(episode, GoldInefficiencyEpisode):
            raise TypeError("episode must be a GoldInefficiencyEpisode")
        ...
        with self._lock:
            ...
            record = {
                "record_type": "gold_inefficiency_episode",
                "schema_version": "1.0",
    ...
    def write_new_closed_episodes(self, tracker) -> Dict[str, Any]:
        if not isinstance(tracker, GoldInefficiencyEpisodeTracker):
            raise TypeError("tracker must be a GoldInefficiencyEpisodeTracker")
